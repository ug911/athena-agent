    ...
    - `data` — `object`
      ...
      - `questions` — `array<object>`
        - `[].data.questions[]` — `object`
          - `correctanswers` — `array<string>`
            - `[].data.questions[].correctanswers[]` — `string`  e.g. `B`, `B`, `C`
      ...
      - `title` — `string`  e.g. `Untitled Quiz`, `Untitled Quiz`, `Exit ticket 1`
      - `type` — `string`  e.g. `QUIZ`, `QUIZ`, `QUIZ`
      - `visibletags` — `array<string>|array<unknown>`
...
LOCATION
  's3://[REDACTED-BUCKET]/processed/wise-app-backend/live_class_agendas/'
TBLPROPERTIES (
  'auto.purge'='false', 
