---
database: processed
table: study_materials_flat_v3
type: view
layer: processed
location: null
format: null
partition_keys: []
last_synced: '2026-04-28T07:12:34+00:00'
sampled_rows: 0
---
...
```sql
CREATE VIEW processed.study_materials_flat_v3 AS
WITH
  data AS (
   SELECT
     userid
   ...
   , (CASE WHEN (CAST("json_extract"(r, '$.file.size["$numberint"]') AS varchar) IS NULL) THEN CAST(0 AS varchar) ELSE CAST("json_extract"(r, '$.file.size["$numberint"]') AS varchar) END) file_size
...
UNION ALL (
      ...
      , CAST("json_extract"(file, '$.size["$numberint"]') AS varchar) file_size
