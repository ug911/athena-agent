...
partition_keys: []
schema_parity: drift
last_synced: '2026-08-11T13:32:03+00:00'
sampled_rows: 0
sampled_region: null
---
...
- Only in `IN`: `licensed`, `location`, `paiduser`
...
```sql
CREATE VIEW processed_na.zoomers_v3 AS
SELECT *
FROM
  (
   SELECT
   ...
   FROM
     (
      ...
      , meetinguuid
      FROM
        "processed_na".wise_app_backend__zoom
   ) 
) 
WHERE (row = 1)
```
...
- **Always filter `row = 1`.** The view emits one row per `zoom_id` per source revision; `row` is a
  `row_number()` over `zoom_id` ordered by `updated_at_full_date_str DESC`, so `row = 1` is the latest state.
- Preferred over `wise_app_backend__zoom` for time-based analysis: `start_time` / `end_time` are already
  typed timestamps, so no Mongo `$date` JSON unwrapping is needed.
- `userid` is the session **host** (the tutor who ran it), which can differ from the class owner —
  see `semantics/glossary.md`.
- `start_time` contains far-future values for `SCHEDULED` / `UPCOMING` / `CANCELLED` rows (dates into the
  2040s+). Filter `meetingstatus = 'ENDED'` for sessions that actually ran, and always bound `start_time`.
